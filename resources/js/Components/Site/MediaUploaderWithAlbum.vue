<script setup>
/**
 * MediaUploaderWithAlbum Component
 * 
 * Enhanced media uploader with album selection and creation.
 * Follows spec requirements for mandatory album association.
 * 
...
/**
 * Load albums and album types
 */
const loadAlbums = async () => {
    try {
        const [albumsResponse, typesResponse] = await Promise.all([
            axios.get('/api/albums'),
            axios.get('/api/album-types')
        ]);
        
        albums.value = albumsResponse.data.data || [];
        albumTypes.value = typesResponse.data.data || [];
    } catch (error) {
...
const createAlbum = async () => {
    if (!newAlbumName.value || !newAlbumType.value) {
        errorMessage.value = 'Nome e tipo do álbum são obrigatórios';
        return;
    }
    ...
    try {
        const response = await axios.post('/api/albums', {
            ...
            album_type_slug: newAlbumType.value,
        });
        ...
        errorMessage.value = error.response?.data?.message || 'Erro ao criar álbum';
    } finally {
...
const processFile = async (file) => {
    ...
    // Validate album selection (mandatory per Requirement 2.3)
    if (!selectedAlbumId.value) {
        errorMessage.value = 'Selecione um álbum antes de fazer upload';
        emit('error', ['Álbum é obrigatório']);
        return;
    }
...
/**
 * Grouped albums by type
 */
const groupedAlbums = computed(() => {
    const groups = {};
    albums.value.forEach(album => {
        const typeName = album.album_type?.name || 'Outros';
        if (!groups[typeName]) {
            groups[typeName] = [];
        }
        groups[typeName].push(album);
    });
    return groups;
});
...
<template>
    <div class="media-uploader-with-album">
        <!-- Album Selection -->
        <div class="album-selection">
            <div class="flex items-center justify-between mb-2">
                <label class="block text-sm font-medium text-gray-700">
                    Álbum <span class="text-red-500">*</span>
                </label>
                <button
                    type="button"
                ...
                >
                    {{ showCreateAlbum ? 'Cancelar' : '+ Criar álbum' }}
                </button>
            </div>
            ...
            <div v-if="showCreateAlbum" class="create-album-form">
                <div class="grid grid-cols-2 gap-3 mb-3">
                    <select
                        ...
                        required
                    >
                        <option value="">Tipo do álbum</option>
                        <option
                ...
                <button
                    type="button"
                    @click="createAlbum"
                    :disabled="!newAlbumName || !newAlbumType || isCreatingAlbum"
                    class="create-album-btn"
                >
                    <span v-if="isCreatingAlbum">Criando...</span>
                    <span v-else>Criar Álbum</span>
                </button>
            </div>
            ...
            >
                ...
                <optgroup
                    v-for="(albumList, typeName) in groupedAlbums"
                    :key="typeName"
                    :label="typeName"
                >
                    <option
        ...
        <div
            ...
            class="upload-zone"
            :class="{ 'dragging': isDragging, 'disabled': !selectedAlbumId }"
            @dragover="handleDragOver"
            @dragleave="handleDragLeave"
            @drop="handleDrop"
            @click="selectedAlbumId ? openFileDialog() : null"
        >
            <input
                ref="fileInputRef"
                type="file"
            ...
            <p class="upload-text">
                <span v-if="selectedAlbumId" class="font-medium text-wedding-600">Clique para selecionar</span>
                <span v-else class="font-medium text-gray-400">Selecione um álbum primeiro</span>
                <span v-if="selectedAlbumId" class="text-gray-500"> ou arraste e solte</span>
            </p>
...
.form-select, .form-input {
    @apply block w-full rounded-md border-gray-300 shadow-sm focus:border-wedding-500 focus:ring-wedding-500 text-sm;
}

.create-album-btn {
    @apply px-3 py-2 text-sm font-medium text-white bg-wedding-600 rounded-md hover:bg-wedding-700 disabled:opacity-50 disabled:cursor-not-allowed;
}
...
.preview-image {
    @apply w-full h-32 object-contain;
}
